...
import { applySettingGlobally } from '@/hooks/useGlobalSettingsWatcher';
...
export function DynamicSettingsManager({ 
  ...
  const { hasAnyPermission, currentUser } = usePermissions();
  ...
  const updateSettingMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: configQueries.dynamicSettings() });
      
      // Apply setting globally using the comprehensive global applier
  ...
  const settings: DynamicSetting[] = category ? getByCategory(category) : [];

  // Filter settings based on permissions
  const visibleSettings = settings.filter((setting: DynamicSetting) => {
    // Check permissions
  ...
  const groupedSettings = visibleSettings.reduce((groups: Record<string, DynamicSetting[]>, setting: DynamicSetting) => {
  ...
  const handleValueChange = (setting: DynamicSetting, value: any) => {
    ...
    // Immediately apply the change via mutation
  ...
  const handleSave = async () => {
    ...
    try {
      // Save all pending changes using our mutation
      ...
      toast.success(`${savePromises.length} settings saved successfully`);
    } catch (error) {
